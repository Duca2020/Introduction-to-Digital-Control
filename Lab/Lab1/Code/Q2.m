...
%      plot(x{i},input_velocity(i)*(step(Gs(i),x{i})));
%      title(strcat('Resposta ao degrau MA (modelo cont nuo) para PWM ',pwm{i},'%'));
%      xlabel('Tempo (s)');
%      ylabel('Velocidade (rps)');
%      legend('velocidade de sa da', 'Location', 'southeast');
%      grid on;
%      saveas(gcf, strcat('C:\Users\franc\OneDrive\Documentos\Lab Controle Digital\1 pratica\Imagens\2)\modelagens\','Resposta ao degrau MA (modelo cont nuo) para PWM ',pwm{i},'%.png'));
%      
%   
%       h = figure(i+4);  
%       hLine1 = line(nan, nan, 'Color','red','linewidth', 2);
%       plot(x{i},input_velocity(i)*step(d2c(Gd(i), 'tustin'),x{i}));
...
%       xlabel('Tempo (s)');
%       ylabel('Velocidade (rps)');
%       legend('Erro de modelagem (modelo cont nuo)', 'Location', 'southeast');
%       title(strcat('Erro de sa da com modelo cont nuo em PWM ', pwm{i}, '%'));
%       grid on
%       saveas(gcf, strcat('C:\Users\franc\OneDrive\Documentos\Lab Controle Digital\1 pratica\Imagens\2)\modelagens\','Erro de sa da com modelo cont nuo em PWM ',pwm{i},'%.png'));
% 
%        h = figure(i+12);  
%       hLine1 = line(nan, nan, 'Color','red','linewidth', 2);
...
%       title(strcat('Erro de sa da com modelo discreto em PWM ', pwm{i}, '%'));
%       grid on
%       saveas(gcf, strcat('C:\Users\franc\OneDrive\Documentos\Lab Controle Digital\1 pratica\Imagens\2)\modelagens\','Erro de sa da com modelo discreto em PWM ',pwm{i},'%.png'));
% 
%     
%       h = figure(i+16);  
%       hLine1 = line(nan, nan, 'Color','red','linewidth', 2);
...
%       xlabel('Tempo (s)');
%       ylabel('Velocidade (rps)');
%       legend('Erro cont nuo - discreto', 'Location', 'southeast');
